...
mod tests {
    ...
    fn test_detect_c_compiler() {
        // On dev machines, a C compiler should typically be available
        let has_cc = detect_c_compiler();
        // Just test it doesn't panic — result depends on environment
        let _ = has_cc;
    }

    #[test]
    fn test_detect_package_manager_macos() {
        if cfg!(target_os = "macos") {
            let result = detect_package_manager("macos");
            // Most macOS dev machines have Homebrew
            // But don't assert specific value — just test it doesn't panic
            let _ = result;
        }
    }
    ...
    fn test_platform_info_serialization() {
        ...
        assert!(json.contains("\"is_wsl\":false"));
        assert!(json.contains("rbenv"));
    }
    ...
    fn test_detect_ruby_managers_returns_vec() {
        ...
        assert!(managers.len() <= 5); // max 5 managers we check
    }
}
