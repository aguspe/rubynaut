...
fn write_config(config: &RubynautConfig) -> Result<(), String> {
    let dir = rubies_dir();
...
fn scan_project_safe(path: &str, name: &str) -> ProjectScanResult {
    let project_dir = PathBuf::from(path);
    if !project_dir.is_dir() {
...
pub async fn bundle_install(
    ...
    let _ = window.emit("bundle-progress", serde_json::json!({
        "path": project_path,
        "status": "running",
        "output": "Running bundle install..."
    }));
...
fn gem_env(ruby_dir: &std::path::Path, ruby_version: &str) -> Vec<(String, String)> {
...
fn build_rubylib(ruby_dir: &std::path::Path) -> String {
...
fn find_gem_spec_dir(ruby_root: &std::path::Path, default: bool) -> Option<PathBuf> {
...
fn read_gemspecs(dir: &std::path::Path, is_default: bool) -> Vec<GemInfo> {
    let mut gems = Vec::new();
...
fn write_cache(versions: &[String]) {
...
async fn fetch_versions_from_github() -> Result<Vec<String>, String> {
...
pub async fn install_ruby(version: String, window: tauri::Window) -> Result<(), String> {
    ...
    let _ = window.emit("install-progress", serde_json::json!({
        "stage": "download",
        "percent": 10,
        "message": format!("Downloading Ruby {version}...")
    }));
    ...
    let _ = window.emit("install-progress", serde_json::json!({
        "stage": "download",
        "percent": 50,
        "message": format!("Downloaded {} MB", bytes.len() / 1_048_576)
    }));
    ...
    let _ = window.emit("install-progress", serde_json::json!({
        "stage": "done",
        "percent": 100,
        "message": format!("Installed: {ruby_version_output}")
    }));
...
fn fix_macos_dylib_paths(target_dir: &std::path::Path, _version: &str) -> Result<(), String> {
...
fn fix_linux_rpath(target_dir: &std::path::Path) -> Result<(), String> {
    let ruby_bin = target_dir.join("bin").join("ruby");
    if !ruby_bin.exists() {
        return Ok(());
    }
...
fn fix_shebangs(target_dir: &std::path::Path) -> Result<(), String> {
...
fn find_ruby_bin_dir(dir: &std::path::Path) -> Option<PathBuf> {
...
fn generate_posix_hook(rubies_path: &str) -> String {
...
fn generate_fish_hook(rubies_path: &str) -> String {
...
fn generate_powershell_hook(rubies_path: &str) -> String {
